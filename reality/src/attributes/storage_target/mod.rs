...
pub mod prelude {
    ...
    ///
    pub(super) type TaskFn<S> = Box<
        dyn FnOnce(&S) -> Pin<Box<dyn Future<Output = ()> + Sync + Send + 'static>>
            + Send
            + Sync
            + 'static,
    >;
    ...
    ///
    pub(super) type MutTaskFn<S> = Box<
        dyn FnOnce(&mut S) -> Pin<Box<dyn Future<Output = ()> + Sync + Send + 'static>>
            + Send
            + Sync
            + 'static,
    >;
    ...
    ///
    pub(super) type OwnedTaskFn<S> = Box<
        dyn FnOnce(S) -> Pin<Box<dyn Future<Output = S> + Sync + Send + 'static>>
            + Send
            + Sync
            + 'static,
    >;
    ...
    ///
    pub(super) type DispatchMutTaskQueue<S> =
        std::sync::Mutex<std::collections::VecDeque<MutTaskFn<S>>>;

    /// Type-alias for a thread safe dispatch owned task queue,
    ///
    pub(super) type DispatchOwnedTaskQueue<S> =
        std::sync::Mutex<std::collections::VecDeque<OwnedTaskFn<S>>>;
