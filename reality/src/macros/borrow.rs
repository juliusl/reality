...
macro_rules! borrow {
    ...
    (async $storage:ident $(,)? $ty:ty, |$var:ident| => $body:block )=> {
        {
            if let Some(resource) = $storage.storage.read().await.resource::<$ty>(ResourceStorageConfig::new()) {
                #[allow(unused_mut)]
                let mut monad = |$var: &$ty| $body;
    ...
    ($storage:ident $(,)?  $ty:ty, $key:literal, |$var:ident| => $body:block )=> {
        {
            const KEY: &'static str = $key;
            if let Some(resource) = $storage.resource::<$ty>(ResourceStorageConfig::new().with_label(KEY)) {
                #[allow(unused_mut)]
                let mut monad = |$var: &$ty| $body;
    ...
    ($storage:ident $(,)? $ty:ty, $key:ident, |$var:ident| => $body:block )=> {
        {
            if let Some(resource) = $storage.resource::<$ty>(ResourceStorageConfig::new().with_hash($key)) {
                #[allow(unused_mut)]
                let mut monad = |$var: &$ty| $body;
...
macro_rules! borrow_mut {
    ...
    ($storage:ident $(,)? $ty:ty, $key:literal, |$var:ident| => $body:block)=> {
        {
            const KEY: &'static str = $key;
            if let Some(mut resource) = $storage.resource_mut::<$ty>(ResourceStorageConfig::new().with_label(KEY)) {
                let monad = |$var: &mut $ty| $body;
    ...
    ($storage:ident $(,)? $ty:ty, $key:ident, |$var:ident| => $body:block)=> {
        {
            if let Some(mut resource) = $storage.resource_mut::<$ty>(ResourceStorageConfig::new().with_hash($key)) {
                let mut monad = |$var: &mut $ty| $body;
...
macro_rules! take {
    (async $storage:ident $(,)? $ty:ty, $key:literal)=> {
        {
            const KEY: &'static str = $key;
            $storage.storage.write().await.take_resource::<$ty>(ResourceStorageConfig::new().with_label(KEY))
        }
    };
    (async $storage:ident $(,)? $ty:ty, $key:ident)=> {
        {
            $storage.storage.write().await.take_resource::<$ty>(ResourceStorageConfig::new().with_hash($key))
        }
    };
    (async $storage:ident $(,)? $ty:ty)=> {
        {
            $storage.storage.write().await.take_resource::<$ty>(ResourceStorageConfig::new())
        }
    };
