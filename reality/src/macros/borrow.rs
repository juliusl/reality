...
macro_rules! borrow {
    (async $storage:ident $(,)? $ty:ty, $key:literal, |$var:ident| => $body:block ) => {{
        ...
        if let Some(resource) = $storage
            ...
            .read()
            .await
            .resource::<$ty>(ResourceKey::with_hash(KEY))
        {
            #[allow(unused_mut)]
    ...
    ($storage:ident $(,)?  $ty:ty, $key:literal, |$var:ident| => $body:block ) => {{
        const KEY: &'static str = $key;
        if let Some(resource) = $storage.resource::<$ty>(ResourceKey::with_hash(KEY)) {
            #[allow(unused_mut)]
            let mut monad = |$var: &$ty| $body;
...
macro_rules! borrow_mut {
    (async $storage:ident $(,)? $ty:ty, $key:literal, |$var:ident| => $body:block) => {{
        ...
        if let Some(mut resource) = $storage
            ...
            .write()
            .await
            .resource_mut::<$ty>(Some(ResourceKey::with_label(KEY)))
        {
    ...
    ($storage:ident $(,)? $ty:ty, $key:literal, |$var:ident| => $body:block) => {{
        const KEY: &'static str = $key;
        if let Some(mut resource) = $storage.resource_mut::<$ty>(ResourceKey::with_hash(KEY)) {
            let monad = |$var: &mut $ty| $body;
...
macro_rules! take {
    (async $storage:ident $(,)? $ty:ty, $key:literal) => {{
        $storage
            ...
            .write()
            .await
            .take_resource::<$ty>(ResourceKey::with_hash($key))
    }};
    ...
    (async $storage:ident $(,)? $ty:ty) => {{
        $storage
            ...
            .write()
            .await
            .take_resource::<$ty>(ResourceKey::root())
    }};
