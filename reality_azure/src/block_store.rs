...
use reality::wire::{
    ...
    BlockBuilder, BlockStore, BlockStoreBuilder, ControlDevice, Encoder,
    Frame, Interner,
};
...
/// Struct for building blocks into block blob,
/// 
            ...
            if let Some(block_order) = block_order.take() {
                for name in block_order {
                    if let Some(mut builder) = builders.remove(&interner.add_ident(name)) {
                        ...
                        block_client
                            .put_block(ext.bytes(), builder.frame_block_data())
                            .await
