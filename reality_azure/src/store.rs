...
use reality::{
    ...
    wire::{ControlDevice, Data, Frame, FrameBuffer, Interner, Protocol, ResourceId, BlockStore},
    Keywords,
};
use std::{
...
use crate::{AzureBlockStore, AzureBlockClient};
...
impl Store {
    ...
    pub async fn take(&mut self, prefix: impl AsRef<str>, timeout: Option<Duration>) -> bool {
        let blob_client = self.get_blob_client(prefix.as_ref());
    ...
    pub async fn index(&self, prefix: impl AsRef<str>) -> Option<StoreIndex<AzureBlockClient>> {
        ...
        let index = AzureBlockStore::new(AzureBlockClient {
            client: blob_client,
