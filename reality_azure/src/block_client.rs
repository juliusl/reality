...
use azure_storage_blobs::{
    blob::{
        operations::{PutBlockBuilder, PutBlockListBuilder},
        BlobBlockWithSize,
    },
    prelude::{BlobClient, BlockId, BlockList, Snapshot},
};
use reality::wire::{block_tasks::ListBlocks, BlockEntry, Frame};
use std::{io::ErrorKind, ops::Range, sync::Arc};
...
impl AzureBlockClient {
                ...
                match resp {
                    Ok(r) => {
                        ...
                        if decompress {
                            ...
                            match tokio::io::copy(&mut reader.as_ref(), &mut writer).await {
                                Ok(copied) => {
                                    event!(Level::TRACE, "Copied {copied} bytes to stream");
                                }
                                Err(err) => {
                                    event!(Level::ERROR, "Error copying bytes to decoder {err}");
                                }
    ...
    pub async fn snapshot(&self) -> Self {
        let resp = self
            ...
            .snapshot()
            .await
            .expect("should be able to take snapshot");
    ...
    pub fn exists(&self) -> bool {
        if !self.exists {
            tokio::runtime::Handle::current()
                .block_on(async { self.client.exists().await.unwrap_or_default() })
        } else {
            true
        }
    }
...
impl reality::wire::BlockClient for AzureBlockClient {
    ...
    fn stream_range(&self, range: Range<usize>) -> Self::Stream {
        self.pull_byte_range(range, true)
    }

    fn transport_range(&self, range: Range<usize>) -> Self::Stream {
        self.pull_byte_range(range, false)
    }
